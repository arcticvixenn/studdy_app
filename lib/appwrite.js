...
export const signIn = async (email, password) => {
  try {
    ...
    throw new Error(error?.message || 'Помилка виконання дії.');
  }
};
  ...
  try {
    ...
    return await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        authorId: user.$id,
        authorName: user.username || user.name || 'Користувач Studdy',
...
const NLP_STOP_WORDS = new Set([
  'Навчальний матеріал', 'Р№', 'Навчальний матеріал', 'Р В°Р В±Р С•', 'Р В°Р В»Р Вµ', 'С‰Рѕ', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Р Р…Р В°', 'Навчальний матеріал', 'Р Р†', 'Р Т‘Р С•',
  'Р В·', 'Навчальний матеріал', 'Р В·Р В°', 'РїСЂРѕ', 'РїСЂРё', 'Навчальний матеріал', 'Р Р…Р В°Р Т‘', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Р В±Р ВµР В·',
  'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Р СР С•Р В¶Р Вµ', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал',
  'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал',
  'Р Р†Р С•Р Р…Р В°', 'Р Р†Р С•Р Р…Р С‘', 'Р Р†Р С•Р Р…Р С•', 'Навчальний матеріал', 'Навчальний матеріал', 'Навчальний матеріал', 'Р№РѕРіРѕ', 'Навчальний матеріал',
...
const toSearchResult = (item, type, query) => {
  ...
  return {
    ...
    reason:
      score > 0
        ? 'Рекомендовано Studdy ML на основі теми, активності та схожості матеріалу.'
        : 'Рекомендовано Studdy ML на основі теми, активності та схожості матеріалу.',
  };
};
...
export const searchContentWithNlp = async ({ query, limit = 30 } = {}) => {
  ...
  if (!cleanedQuery) {
    return {
      query: '',
      modelType: 'NLP-схожість тексту search',
      results: [],
    };
  }
...
export async function saveMlGeneratedQuiz({
  ...
  const quizDocument = await createQuiz({
    courseId,
    lessonId,
    title: title || 'Навчальний матеріал',
    passingScore: 60,
    user,
  });
...
export const getUserGamificationProfile = async (userId) => {
  ...
  const getRank = (level) => {
    ...
    if (level >= 8) return 'Навчальний матеріал';
    if (level >= 4) return 'Навчальний матеріал';
  ...
  try {
    ...
    return {
      ...
      achievements: [
        {
          icon: 'СЂСџР‹Р‡',
          title: 'Навчальний матеріал',
          description: 'Навчальний матеріал',
        ...
        {
          icon: 'Матеріал підібрано для покращення навчального прогресу.',
          title: 'Навчальний матеріал',
          description: 'Навчальний матеріал',
...
export const getOrCreateUserProgress = async (userId) => {
  ...
  if (existing.documents.length > 0) {
    ...
    const levelInfo = calculateLevelInfo(progress.xpTotal || 0);
...
export const awardXp = async ({
  ...
  const nextLevelInfo = calculateLevelInfo(nextXpTotal);
...
export const uploadUserAvatar = async ({ user, asset }) => {
  if (!user?.$id) {
    throw new Error('Користувача не знайдено.');
  }

  if (!asset?.uri) {
    throw new Error('Фото не вибрано.');
  }
  ...
  const fileName =
    asset.fileName ||
    `avatar_${user.$id}_${Date.now()}.${fileExtension}`;
  ...
  const avatarUrl =
    `${endpoint}/storage/buckets/${storageId}/files/${uploadedFile.$id}/view?project=${projectId}`;
  ...
  try {
    ...
    console.log('update post avatars error:', error);
  }
