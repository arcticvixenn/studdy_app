    ...
    export const searchPosts = async (query) => {
        try {
            const posts = await databases.listDocuments(
                databaseId,
                videoCollectionId,
                [Query.search("title", query)] // передаємо поле та значення запиту
            );
            return posts.documents;
        } catch (error) {
            throw new Error(error);
    ...
    export const getUserPosts = async (userId) => {
        try {
            const posts = await databases.listDocuments(
                databaseId,
                videoCollectionId,
                [Query.equal("creator", userId),  Query.orderDesc("$createdAt")] // передаємо поле та значення запиту
            );
            return posts.documents;
        } catch (error) {
            throw new Error(error);
