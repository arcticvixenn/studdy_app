...
const backup = `${path}.bak.real_xp_events_${Date.now()}`;
...
// helper для автоматичного зарахування квесту дня
if (!content.includes("export const completeDailyQuestIfMatches")) {
  ...
  if (insertIndex === -1) {
    throw new Error("awardQuizXp marker not found");
  }
...
// XP за проходження тесту
if (!content.includes("XP_QUIZ_AWARD_PATCH")) {
  content = replaceBlock(
    content,
    "export const submitQuizAttempt = async ({",
    (block) => {
      const lastReturnIndex = block.lastIndexOf("return ");

      if (lastReturnIndex === -1) {
        throw new Error("No return found in submitQuizAttempt");
      }
...
`;
      ...
      return block.slice(0, lastReturnIndex) + injection + block.slice(lastReturnIndex);
    }
  );
}
...
console.log("DONE: real XP events patched");
